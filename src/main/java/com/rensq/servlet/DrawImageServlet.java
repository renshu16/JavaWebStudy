...
public class DrawImageServlet extends HttpServlet {
    ...
    protected void doPost(HttpServletRequest req, HttpServletResponse resp) throws ServletException, IOException {
        ...
        grap.drawRect(1,1,WIDTH-2,HEIGHT-2);
        ...
        //绘制随机字符
        String randomNumber = drawRandomNumber((Graphics2D) grap, flag);

        //将随机数存在session中
        req.getSession().setAttribute("checkcode",randomNumber);
        resp.setContentType("image/jpeg");
        resp.setDateHeader("expries",-1);
        resp.setHeader("Cache-Control","no-cache");
        resp.setHeader("Pragma","no-cache");
        ImageIO.write(imageBuffer,"jpg",resp.getOutputStream());

    }

    private String drawRandomNumber(Graphics2D grap,String... createTypeFlag){
        ...
        if (createTypeFlag.length >0 && null != createTypeFlag[0]){
            if (createTypeFlag[0].equals("ch")){
                return createRandomChar(grap,baseChineseChar);
            }else if(createTypeFlag[0].equals("nl")){
                return createRandomChar(grap,baseNumLetter);
            }else if(createTypeFlag[0].equals("n")){
                return createRandomChar(grap,baseNum);
            }else if(createTypeFlag[0].equals("l")){
                return createRandomChar(grap,baseLetter);
            }
        }else{
            return createRandomChar(grap,baseNumLetter);
        }
    ...
    private String createRandomChar(Graphics2D grap,String baseChar){
        ...
        for (int i=0; i<4; i++){
            ...
            sb.append(ch);
            //正向角度
            grap.rotate(degree * Math.PI/180,x,20);
            grap.drawString(ch,x,20);
            //反向角度
            grap.rotate(-degree * Math.PI / 180, x, 20);
            x += 30;
